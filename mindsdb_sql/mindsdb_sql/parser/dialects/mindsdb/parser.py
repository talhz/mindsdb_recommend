...
from mindsdb_sql.parser.dialects.mindsdb.recommend_model import RecommendModel
...
class MindsDBParser(Parser):
    ...
    @_('RECOMMEND identifier')
    def recommend(self, p):
        return Recommend(value=p.identifier)
    
    @_('RECOMMEND MODEL if_not_exists_or_empty identifier FROM identifier LPAREN raw_query RPAREN PREDICT result_columns')
    def recommend_model(self, p):
        query_str = None
        if hasattr(p, 'raw_query'):
            query_str = tokens_to_string(p.raw_query)
